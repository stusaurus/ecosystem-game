...
(function(){
  ...
  function renderDetail(){
    ...
    body.innerHTML=`<div class="v3-section"><div class="v3-title">🏁 クリア条件</div>${items.map(x=>`<div class="v3-check">${x.ok?'✅':'⬜️'} ${x.text} <small>${x.now}</small></div>`).join('')}</div><div class="v3-section"><div class="v3-title">🐾 個体数</div><div class="v3-grid">${order.map(k=>{const d=SPECIES[k]||PLANTS[k];return `<div class="v3-count">${d.icon} ${d.name}<b>${c[k]||0}</b></div>`;}).join('')}</div></div><div class="v3-section"><div class="v3-title">♻️ 命の循環</div>土の栄養：<b>${nutrient()}</b><br>植物 → 草食動物 → 肉食動物 → 死 → 土 → 植物</div>`;
  }

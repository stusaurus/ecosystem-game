...
(function(){
  ...
  function inject(){
    ...
    style.textContent=`
      .aid-sheet{position:fixed;inset:0;z-index:120;background:rgba(25,42,51,.58);display:grid;place-items:center;padding:12px}
      .aid-sheet.hidden{display:none!important}
      .aid-card{width:min(520px,100%);max-height:calc(100dvh - 24px);overflow:auto;background:#fffdf8;border:1px solid #d4e1e7;border-radius:20px;padding:14px;box-shadow:0 24px 60px rgba(20,35,44,.3)}
      ...
      .aid-x{width:42px;height:42px;border:1px solid #d4e1e7;border-radius:12px;background:white;font-size:20px;font-weight:900}
      ...
      .aid-animal{min-height:58px;border:1px solid #d4e1e7;border-radius:13px;background:white;text-align:left;padding:9px;color:#20313a}
      ...
      .aid-back{width:100%;min-height:48px;margin-top:10px;border:0;border-radius:13px;background:#29485b;color:white;font-size:13px;font-weight:900}
  ...
  function render(){
    ...
    body.querySelectorAll('[data-sheet-aid]').forEach(btn=>btn.addEventListener('click',()=>request(btn.dataset.sheetAid)));
  }
  ...
  function request(type){
    ...
    addEntities(type);
    const accepted=state.interventionPoints<beforePoints||state.pendingInterventions.length>beforePending;
    if(accepted){
      const d=SPECIES[type];
      el('aidSheet').classList.add('hidden');
      resume();
      ticker(`${d.icon}${d.name}の介入を受け付けました。約${AID_DELAY[type].toFixed(1)}年後に到着します。`);
    }else{
      render();
    }
  }
